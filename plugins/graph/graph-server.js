...
const { initializeGraphDatabase, getGraphDb, populateGraphSequenceIds } = require("./graph-database");
...
// Middleware
app.use(cors({
  origin: ['http://localhost:3003', 'http://localhost:3000', 'http://127.0.0.1:3003', 'http://127.0.0.1:3000'],
  credentials: true
}));
...
app.post("/api/graph/nodes", async (req, res) => {
  try {
    ...
    const node = await req.db.get("SELECT * FROM graph_nodes WHERE id = ?", nodeId);
    res.json(node);
  } catch (error) {
...
app.post("/api/graph/edges", async (req, res) => {
  try {
    ...
    const edge = await req.db.get("SELECT * FROM graph_edges WHERE id = ?", edgeId);
    res.json(edge);
  } catch (error) {
