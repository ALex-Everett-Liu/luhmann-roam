...
const HotkeyManager = (function () {
    ...
    mutationObserver = new MutationObserver((mutations) => {
      ...
      mutations.forEach((mutation) => {
        ...
        if (
          ...
          mutation.target.closest(".modal-body, .modal-header, .modal-footer")
        ) {
          return;
        }
  ...
  function handleKeyDown(e) {
    ...
    if (isHotkeyModeActive) {
      ...
      if (/^[1-9]$/.test(key)) {
        ...
        const hasTwoDigitHotkeys = Object.keys(hotkeys).some(
          (k) => k.length === 2 && k.startsWith(key) && hotkeys[k].isTwoDigit,
        );
  ...
  function registerNodeNavigationHotkeys() {
    ...
    if (isFocusedView) {
      ...
      nodesToNumber = Array.from(document.querySelectorAll(".node")).filter(
        (node) => {
      ...
      console.log(
        `Found ${nodesToNumber.length} visible nodes in focused view`,
      );
    } else {
      // At root level: only show first node
      nodesToNumber = Array.from(
        document.querySelectorAll("#outliner-container > .node"),
      );
      ...
      console.log(
        `Found ${nodesToNumber.length} root nodes, showing ${nodesToNumber.length} numbered hints`,
      );
    }
  ...
  function registerBreadcrumbHotkeys() {
    ...
    const breadcrumbItems = Array.from(
      document.querySelectorAll(
        ".breadcrumb-item:not(.breadcrumb-home):not(.breadcrumb-active)",
      ),
    );
  ...
  function registerNodeActionHotkeys() {
    ...
    const visibleNodeActions = Array.from(
      document.querySelectorAll(".node-content:hover .node-actions"),
    );
  ...
  function registerGlobalHotkeys() {
    ...
    globalHotkeys["f"] = {
      ...
      action: () => {
        ...
        if (!nodeToFocus) {
          ...
          if (focusedTextElement) {
            ...
            if (nodeElement && nodeElement.dataset.id) {
              ...
              console.log(
                "Using currently focused text element node:",
                nodeToFocus,
              );
            }
          }
        ...
        if (nodeToFocus) {
          ...
          const nodeElement = document.querySelector(
            `.node[data-id="${nodeToFocus}"]`,
          );
          if (nodeElement) {
            console.log(
              "Node found in DOM, using BreadcrumbManager.focusOnNode",
            );
            ...
            console.log(
              "Node not found in current DOM, may need to navigate to it first",
            );
          ...
          alert(
            "No node to focus on. Try clicking on a node first, or use search (Alt+S) to find a specific node.",
          );
        }
      },
    };
    ...
    globalHotkeys["z"] = {
      ...
      action: () => {
        console.log(
          "Alt+Z pressed - attempting to focus on last focused node text",
        );
        console.log("window.lastFocusedNodeId:", window.lastFocusedNodeId);
