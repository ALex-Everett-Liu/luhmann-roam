  ...
  // debugToggleButton();
    ...
    async function showImageDetail(imageId) {
      try {
        ...
        // Load and display subsidiary images
        loadSubsidiaryImages(image.id);

        // Add event listeners for subsidiary buttons
        document.getElementById('dcim-add-subsidiary').addEventListener('click', () => showAddSubsidiaryForm(image.id));
        document.getElementById('dcim-manage-subsidiaries').addEventListener('click', () => showManageSubsidiariesDialog(image.id));
    ...
    // Add the following functions after saveImageChanges
    ...
    try {
      ...
      container.querySelectorAll('.view-subsidiary').forEach(btn => {
        btn.addEventListener('click', () => {
          ...
          showImageDetail(btn.dataset.id);
        });
      });
