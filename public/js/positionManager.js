...
const PositionManager = (function () {
  ...
  async function openPositionAdjustModal(nodeId) {
    const { modalOverlay, positionInput, currentPositionInfo } =
      createPositionAdjustModal(nodeId);
  ...
  async function adjustNodePosition(nodeId, newPosition) {
    try {
      ...
      if (
        window.NodeOperationsManager &&
        window.NodeOperationsManager.refreshSubtree
      ) {
        if (node.parent_id) {
  ...
  function createMoveNodeModal(nodeId) {
    ...
    moveButton.addEventListener("click", () => {
      moveNodeToParent(
        ...
        parseInt(positionInput.value, 10),
      );
    });
  ...
  async function openMoveNodeModal(nodeId) {
    const { modalOverlay, selectedNodeInput, positionInput } =
      createMoveNodeModal(nodeId);
    document.body.appendChild(modalOverlay);
    ...
    try {
      ...
      if (node.parent_id) {
        ...
        const parentContent =
          currentLanguage === "en"
            ? parentNode.content
            : parentNode.content_zh || parentNode.content;
      ...
      if (
        ...
        if (originalParentId) {
          refreshPromises.push(
            window.NodeOperationsManager.refreshSubtree(originalParentId),
          );
        }
        ...
        if (parentId && parentId !== originalParentId) {
          refreshPromises.push(
            window.NodeOperationsManager.refreshSubtree(parentId),
          );
        }
  ...
  async function fixNodePositions(nodeId) {
    try {
      ...
      siblings.forEach((sibling) => {
        positionCounts[sibling.position] =
          (positionCounts[sibling.position] || 0) + 1;
      });

      const hasDuplicates = Object.values(positionCounts).some(
        (count) => count > 1,
      );
      ...
      if (hasDuplicates) {
        ...
        for (let i = 0; i < siblings.length; i++) {
          ...
          console.log(
            `Setting node ${sibling.id} position from ${sibling.position} to ${i}`,
          );
