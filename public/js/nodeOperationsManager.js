...
const NodeOperationsManager = (function () {
  ...
  async function addRootNode(nodesArray) {
    // It takes one parameter, nodesArray, which is expected to be an array of existing root nodes. This array is used to determine the position of the new root node.
    try {
      // Get the highest position
      const maxPosition =
        nodesArray && nodesArray.length > 0
        ...
        if (nodeElement) {
          ...
          if (parentElement && parentElement.classList.contains("children")) {
            ...
            if (remainingChildren.length === 0) {
              ...
              if (
                parentNodeElement &&
                parentNodeElement.classList.contains("node")
              ) {
                // Find the collapse icon and replace it with a bullet
                const collapseIcon =
                  parentNodeElement.querySelector(".collapse-icon");
                if (collapseIcon) {
  ...
  async function indentNode(nodeId) {
    try {
      ...
      const wasTextFocused =
        focusedElement && focusedElement.classList.contains("node-text");
      ...
      if (
        wasTextFocused &&
        focusedElement.closest(".node")?.dataset.id === nodeId
      ) {
      ...
      if (wasTextFocused) {
        ...
        setTimeout(() => {
          const newNodeElement = document.querySelector(
            `.node[data-id="${nodeId}"]`,
          );
          if (newNodeElement) {
    ...
    try {
      ...
      if (grandparentContainer.classList.contains("children")) {
        ...
        if (parentIndex >= 0 && parentIndex < siblings.length - 1) {
          ...
          grandparentContainer.insertBefore(
            nodeElement,
            siblings[parentIndex + 1],
          );
        } else {
          // Insert at the end
      ...
      const formerParentChildrenContainer =
        currentParent.querySelector(".children");
      if (
        formerParentChildrenContainer &&
        formerParentChildrenContainer.children.length === 0
      ) {
  ...
  async function moveNodeUp(nodeId) {
    try {
      ...
      if (parentId) {
        ...
        nodeElement &&
        nodeElement.parentElement.id === "outliner-container"
      ) {
  ...
  async function refreshSubtree(nodeId) {
    try {
      ...
      const childrenResponse = await fetch(
        `/api/nodes/${nodeId}/children?_=${Date.now()}`,
      );
      const children = await childrenResponse.json();
...
if (
  ...
  console.log(
    "DOM already ready, initializing NodeOperationsManager immediately",
  );
  NodeOperationsManager.initialize();
}
