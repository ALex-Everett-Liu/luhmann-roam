...
(function () {
  const PluginManager = {
    ...
    registerPlugin: function (id, options = {}) {
      ...
      console.log(
        `Plugin "${id}" registered with state: ${pluginOptions.enabled ? "enabled" : "disabled"}`,
      );
    ...
    registerBuiltInPlugins: function () {
      ...
      this.registerPlugin("localGraphManager", {
        ...
        description:
          "Explore local graph neighborhoods around a center node with distance-based filtering",
        defaultEnabled: true,
        category: "analysis",
      });
      ...
      this.registerPlugin("globalGraphManager", {
        ...
        description:
          "Complete graph visualization with multiple layouts and centrality analysis algorithms",
        defaultEnabled: true,
        category: "analysis",
      });
      ...
      this.registerPlugin("codeGraphManager", {
        ...
        description:
          "Complex code analysis and visualization tool (legacy version) - May cause cursor issues",
        ...
        cleanup: function () {
          if (window.CodeGraphManager) {
            ...
            document.removeEventListener(
              "click",
              window.codeGraphDropdownHandler,
            );
            document.removeEventListener(
              "click",
              window.codeGraphAutocompleteHandler,
            );
        ...
        toggle: function (enabled) {
          if (window.CodeGraphManager) {
            if (enabled) {
              console.warn(
                "⚠️ Enabling legacy CodeGraphManager - this may cause cursor issues in the main outliner",
              );
              CodeGraphManager.initialize();
            } else {
              console.log(
                "🧹 Disabling legacy CodeGraphManager and cleaning up global listeners",
              );
              CodeGraphManager.hide();
      ...
      this.registerPlugin("enhancedCodeGraphManager", {
        ...
        description:
          "Full CRUD code analysis with project management and advanced features",
        defaultEnabled: true,
        requiresReload: false,
        category: "analysis",
        initialize: function () {
    ...
    loadPluginStates: function () {
      ...
      const pluginStateKeys = keys.filter(
        (key) => key.startsWith("plugin_") && key.endsWith("_enabled"),
      );
    ...
    togglePlugin: function (id, enabled) {
      ...
      console.log(
        `Toggling plugin "${id}" to: ${newState ? "enabled" : "disabled"}`,
      );
      ...
      if (newState && plugin.dependencies && plugin.dependencies.length > 0) {
        for (const depId of plugin.dependencies) {
          if (!this.isPluginEnabled(depId)) {
            console.warn(
              `Plugin "${id}" requires "${depId}" which is disabled. Enabling dependency.`,
            );
      ...
      if (
        ...
        typeof this.onPluginStateChange === "function"
      ) {
      ...
      if (plugin.requiresReload) {
        alert(
          `The ${plugin.name} plugin requires a page reload to ${newState ? "enable" : "disable"} fully.`,
        );
      }
