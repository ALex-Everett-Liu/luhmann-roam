...
const StyleSettingsManager = (function () {
  ...
  function initialize() {
    ...
    console.log(
      "Style Settings Manager initialized with settings:",
      currentSettings,
    );
  }
  ...
  function saveAppearanceSettings() {
    ...
    if (themeToggler) {
      themeToggler.textContent =
        currentSettings.theme === "dark" ? "☀️ Light Mode" : "🌙 Dark Mode";
      themeToggler.title =
  ...
  function addThemeToggler() {
    ...
    themeToggler.addEventListener("click", () => {
      ...
      currentSettings.theme =
        currentSettings.theme === "dark" ? "light" : "dark";
  ...
  function markActiveColor(containerId, color) {
    ...
    const matchingBtn = Array.from(
      container.querySelectorAll(".preset-color"),
  ...
  function updateSettingsFromForm() {
    ...
    if (bgOpacitySlider) {
      currentSettings.backgroundImage.opacity = parseFloat(
        bgOpacitySlider.value,
      );
    }
    ...
    if (contentColorAdjust) {
      currentSettings.backgroundImage.contentColorAdjust =
        contentColorAdjust.value;
    }
  ...
  function applySettings() {
    ...
    if (
      currentSettings.backgroundImage.enabled &&
      currentSettings.backgroundImage.url
    ) {
      document.documentElement.style.setProperty(
        "--background-image-url",
        `url(${currentSettings.backgroundImage.url})`,
      );
      document.documentElement.style.setProperty(
        "--background-image-overlay",
        currentSettings.backgroundImage.opacity,
      );
      document.documentElement.style.setProperty(
        "--background-image-blur",
        `${currentSettings.backgroundImage.blur}px`,
      );
      ...
      if (colorAdjust === "auto") {
        ...
        document.documentElement.style.setProperty(
          "--custom-text-color",
          currentSettings.backgroundImage.customTextColor,
        );
        document.documentElement.style.setProperty(
          "--custom-bg-color",
          currentSettings.backgroundImage.customBgColor,
        );
      }
    } else {
  ...
  function setupBackgroundImageControls() {
    ...
    const contentColorAdjust = document.getElementById("content-color-adjust");
    const customColorsContainer = document.getElementById(
      "custom-colors-container",
    );
    const customTextColor = document.getElementById("custom-text-color");
    const customTextColorPreview = document.getElementById(
      "custom-text-color-preview",
    );
    const customBgColor = document.getElementById("custom-bg-color");
    const customBgColorPreview = document.getElementById(
      "custom-bg-color-preview",
    );
    const customBgOpacity = document.getElementById("custom-bg-opacity");
    const customBgOpacityLabel = document.getElementById(
      "custom-bg-opacity-label",
    );
  ...
  function updateBackgroundPreview() {
    ...
    const opacity = bgOpacitySlider
      ? bgOpacitySlider.value
      : currentSettings.backgroundImage.opacity;
    const blur = bgBlurSlider
      ? bgBlurSlider.value
      : currentSettings.backgroundImage.blur;
  ...
  function updateContentPreview() {
    ...
    if (!contentPreview) {
      ...
      bgPreview.parentNode.insertBefore(
        previewContainer,
        bgPreview.nextSibling,
      );
    }
