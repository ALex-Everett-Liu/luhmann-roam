      ...
      for (const [tableName, records] of Object.entries(data)) {
        ...
        for (const record of records) {
          try {
            ...
            result.errors.push(
              `Error importing record in ${tableName}: ${error.message}`,
            );
          }
        }
...
exports.listTables = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to list database tables: " + error.message });
  }
};
...
exports.getTableSchema = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to get table schema: " + error.message });
  }
};
...
exports.exportNodeTree = async (req, res) => {
  try {
    const { nodeId, includeTables = ["nodes", "links", "node_attributes"] } =
      req.body;
    ...
    res
      .status(500)
      .json({ error: "Failed to export node tree: " + error.message });
  }
};
